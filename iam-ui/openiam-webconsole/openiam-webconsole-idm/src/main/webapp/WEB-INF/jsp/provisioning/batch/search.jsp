...
<html>
    <head>
		...
		<title>${titleOrganizatioName} - <fmt:message key="openiam.ui.webconsole.idm.batch.tasks.title"/></title>
		...
		<script type="text/javascript" src="/openiam-ui-static/js/webconsole-idm/batch/batch.task.search.js"></script>
		<script type="text/javascript">
			OPENIAM = window.OPENIAM || {};
			OPENIAM.ENV = window.OPENIAM.ENV || {};
			OPENIAM.ENV.MenuTree = <c:choose><c:when test="${! empty requestScope.menuTree}">${requestScope.menuTree}</c:when><c:otherwise>null</c:otherwise></c:choose>;
			OPENIAM.ENV.Text = {
				EmptyResults : localeManager["openiam.ui.webconsole.idm.batch.tasks.notFound"]
			}
		</script>
	</head>
	<body>
		<div id="title" class="title">
            <fmt:message key="openiam.ui.webconsole.idm.batch.tasks.title"/>
		</div>
