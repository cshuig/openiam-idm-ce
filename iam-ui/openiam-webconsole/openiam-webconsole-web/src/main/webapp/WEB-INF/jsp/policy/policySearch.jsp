...
<html>
    <head>
		...
		<title>${titleOrganizatioName}</title>
		...
		<script type="text/javascript" src="/openiam-ui-static/js/webconsole/policy/policy.search.js"></script>
		<script type="text/javascript">
			OPENIAM = window.OPENIAM || {};
			OPENIAM.ENV = window.OPENIAM.ENV || {};
			OPENIAM.ENV.MenuTree = <c:choose><c:when test="${! empty requestScope.menuTree}">${requestScope.menuTree}</c:when><c:otherwise>null</c:otherwise></c:choose>;
			OPENIAM.ENV.PolicyDefId = "${requestScope.policyDefId}";
			OPENIAM.ENV.PolicyPageURL = "${requestScope.policyPageURL}";
			OPENIAM.ENV.Text = {
				EmptyResults : localeManager["openiam.ui.webconsole.policy.notFound"]
			}
		</script>
	</head>
	<body>
		<div id="title" class="title">
			<c:choose>
				<c:when test="${requestScope.policyType eq 'PASSWORD'}">
                    <fmt:message key="openiam.ui.webconsole.policy.password.title" />
				</c:when>
				<c:when test="${requestScope.policyType eq 'AUTHENTICATION'}">
                    <fmt:message key="openiam.ui.webconsole.policy.authentication.title" />
				</c:when>
				<c:when test="${requestScope.policyType eq 'ATTRIBUTE'}">
                    <fmt:message key="openiam.ui.webconsole.policy.attribute.title" />
				</c:when>
			</c:choose>
		</div>
