...
<c:set var="pageTitle">
	<fmt:message key="openiam.ui.shared.resource.search.page.title" />
</c:set>
...
<html>
	<head>
		...
		<script type="text/javascript" src="/openiam-ui-static/js/common/resource/resource.search.js"></script>
		...
		<script type="text/javascript">
			...
			OPENIAM.ENV.ResourceTypes = <c:choose><c:when test="${! empty requestScope.resourceTypeList}">${requestScope.resourceTypeList}</c:when><c:otherwise>null</c:otherwise></c:choose>;
			OPENIAM.ENV.TargetResourceType = <c:choose><c:when test="${! empty requestScope.targetResourceType}">"${requestScope.targetResourceType}"</c:when><c:otherwise>null</c:otherwise></c:choose>;
			OPENIAM.ENV.ExcludeResourceType = <c:choose><c:when test="${! empty requestScope.excludeResourceType}">"${requestScope.excludeResourceType}"</c:when><c:otherwise>null</c:otherwise></c:choose>;
		</script>
	</head>
	<body>
