...
<head>
    ...
    <title>${titleOrganizatioName} -
        <c:choose>
            <c:when test="${requestScope.type eq 'superiors'}">
                <fmt:message key="openiam.ui.user.sup.page.title"/>
            </c:when>
            <c:when test="${requestScope.type eq 'subordinates'}">
                <fmt:message key="openiam.ui.user.sub.page.title"/>
            </c:when>
        </c:choose>
    </title>
    ...
    <script type="text/javascript" src="/openiam-ui-static/js/common/user/supsub.entitlements.js"></script>
...
<div id="title" class="title">
    <c:choose>
        <c:when test="${requestScope.type eq 'superiors'}">
            <fmt:message key="openiam.ui.user.sup.title"/>: ${requestScope.user.firstName} ${requestScope.user.lastName}
        </c:when>
        <c:when test="${requestScope.type eq 'subordinates'}">
            <fmt:message key="openiam.ui.user.sub.title"/>: ${requestScope.user.firstName} ${requestScope.user.lastName}
        </c:when>
    </c:choose>
    <div id="usermenu">
        <li>
            <a <c:if test="${requestScope.type eq 'superiors'}">class="active"</c:if> href="supSub.html?id=${requestScope.user.id}&type=superiors"><fmt:message key="openiam.ui.user.sup.button"/></a>
        </li>
        <li>
            <a <c:if test="${requestScope.type eq 'subordinates'}">class="active"</c:if> href="supSub.html?id=${requestScope.user.id}&type=subordinates"><fmt:message key="openiam.ui.user.sub.button"/></a>
        </li>
    </div>
</div>
