...
<head>
    ...
    <title>${titleOrganizatioName} - <fmt:message key="openiam.ui.org.identities.page.title"/></title>

    <link href="/openiam-ui-static/css/common/style.css" rel="stylesheet" type="text/css" media="screen" />
    <link href="/openiam-ui-static/css/common/style.client.css" rel="stylesheet" type="text/css" />
    <link href="/openiam-ui-static/plugins/tablesorter/themes/yui/style.css" rel="stylesheet" type="text/css" />
    <link href="/openiam-ui-static/js/common/jquery/css/smoothness/jquery-ui-1.9.1.custom.css" rel="stylesheet" type="text/css" />
    <link href="/openiam-ui-static/css/webconsole/user.identity.css" rel="stylesheet" type="text/css" />
    ...
    <script type="text/javascript" src="/openiam-ui-static/js/webconsole/organization/org.identities.js"></script>
    ...
    <script type="text/javascript">
        ...
        OPENIAM.ENV.OrgId = <c:choose><c:when test="${! empty requestScope.org.id}">"${requestScope.org.id}"</c:when><c:otherwise>null</c:otherwise></c:choose>;
        OPENIAM.ENV.MenuTreeAppendURL = <c:choose><c:when test="${! empty requestScope.org.id}">"id=${requestScope.org.id}"</c:when><c:otherwise>null</c:otherwise></c:choose>;
        OPENIAM.ENV.ManagedSystems = <c:choose><c:when test="${! empty requestScope.managedSystems}">${requestScope.managedSystems}</c:when><c:otherwise>null</c:otherwise></c:choose>;
        OPENIAM.ENV.DateFormatDP = "${requestScope.dateFormatDP}";
    </script>
</head>
<body>
<div id="title" class="title">
    <fmt:message key="openiam.ui.org.identities"/> : ${requestScope.org.name}
</div>
