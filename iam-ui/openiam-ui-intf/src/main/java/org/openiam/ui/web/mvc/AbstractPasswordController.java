...
public abstract class AbstractPasswordController extends AbstractLoginController implements InitializingBean {
    ...
    @Value("${org.openiam.ui.user.reset.password.option.sendByEmail.show}")
    protected boolean sendByEmailShow;
    @Value("${org.openiam.ui.user.reset.password.option.sendByEmail.defaultValue}")
    protected boolean sendByEmailDefault;

    @Value("${org.openiam.ui.user.reset.password.option.autoGenerate.show}")
    protected boolean autoGenerateShow;
    @Value("${org.openiam.ui.user.reset.password.option.autoGenerate.defaultValue}")
    protected boolean autoGenerateDefault;

    @Value("${org.openiam.ui.user.reset.password.button.reSync.show}")
    protected boolean reSyncShow;

    @Value("${org.openiam.ui.user.reset.password.displayAfterReset}")
    protected boolean displayPasswordAfterReset;

    @Value("${org.openiam.ui.user.reset.password.defaultManagedSysIds}")
    protected String resetPasswordDefaultManagedSysIds;
    ...
    protected SetPasswordToken attemptResetPassword(final HttpServletRequest request,
        ...
        passwordSync.setManagedSystemId(managedSystemId);
    ...
    protected SetPasswordToken attemptResetPassword(final PasswordSync passwordSync) {
        ...
        if (response.isFailure()) {
            ...
            if (!token.hasErrors()) {
                token.addError(new ErrorToken(Errors.RESYNC_PASSWORD_FAILED));
            }
