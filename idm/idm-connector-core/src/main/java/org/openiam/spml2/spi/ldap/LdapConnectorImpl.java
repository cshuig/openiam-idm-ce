			...
			for (ExtensibleObject obj : responseType.getAny()) {
				...
				if (principal != null) {
					...
					if (login == null) {
						...
						/**
						 * We should Add it to IDM Use case 1: We want to
						 * reconcile from AD back to IDM In the case AD has lots
						 * of users and IdM is empty (no users, but we have
						 * defined Groups and Roles) Under this situation, The
						 * reconciliation situation: "IDM NOT FOUND" so
						 * "CREATE IDM ACCOUNT"
						 **/
						// /**
						// * TODO REMOVE AFTER REVIEW DeleteRequestType delete =
						// * new DeleteRequestType(); idType = new
						// * PSOIdentifierType(searchPrincipal, null,
						// * managedSysId); delete.setPsoID(idType);
						// * delete(delete); Login l = new Login(); LoginId id =
						// * new LoginId(); id.setDomainId(mSys.getDomainId());
						// * id.setLogin(principal);
						// * id.setManagedSysId(managedSysId); // l.setId(id);
						// **/
