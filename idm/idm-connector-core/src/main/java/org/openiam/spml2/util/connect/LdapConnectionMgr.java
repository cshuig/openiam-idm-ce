...
public class LdapConnectionMgr implements ConnectionMgr {
	...
	public LdapContext connect(ManagedSys managedSys) throws NamingException {
		...
		if (managedSys.getPort() != null && managedSys.getPort() > 0) {
			hostUrl = hostUrl + ":" + String.valueOf(managedSys.getPort());
		}

		log.debug("connect: Connecting to target system: "
		...
		// log.info(" directory login passowrd= " +
        ...
        */
        try {
			...
			if (secondarySysID != null && !secondarySysID.isEmpty()) {
				...
				// recursively search through the chained list of linked managed
				// systems
				ManagedSystemDataService managedSysService = (ManagedSystemDataService) ac
						.getBean("managedSysService");
				ManagedSys secondarySys = managedSysService
						.getManagedSys(secondarySysID);
