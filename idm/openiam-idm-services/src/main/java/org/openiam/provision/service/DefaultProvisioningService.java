...
public class DefaultProvisioningService extends AbstractProvisioningService
	...
	public ProvisionUserResponse addUser(ProvisionUser user) {
		...
		if (passwordPolicy == null) {
			passwordPolicy = passwordDS.getPasswordPolicyByUser(primaryLogin
					.getId().getDomainId(), user.getUser());
		}
		...
		if (customPassword) {
			...
			try {
				PasswordValidationCode valCode = passwordDS
						.isPasswordValidForUserAndPolicy(password,
								user.getUser(), primaryLogin, passwordPolicy);
				if (valCode == null
					...
					auditHelper.addLog("CREATE", user.getRequestorDomain(),
							...
							ResponseCode.FAIL_DECRYPTION.toString(), user
									.getSessionId(),
							"Password validation failed", user
									.getRequestClientIP(), primaryLogin.getId()
									.getLogin(), primaryLogin.getId()
									.getDomainId());
				...
				auditHelper
						.addLog("CREATE", user.getRequestorDomain(), user
								...
								primaryLogin.getId().getLogin(), primaryLogin
										.getId().getDomainId());
		...
		Login dupPrincipal = loginManager.getLoginByManagedSys(
				dupId.getDomainId(), dupId.getLogin(), dupId.getManagedSysId());
		...
		if (dupPrincipal != null) {
			...
			auditHelper.addLog("CREATE", user.getRequestorDomain(),
					...
					user.getSessionId(),
					"Identity already exists:" + dupId.getManagedSysId()
							+ " - " + dupId.getLogin(),
					user.getRequestClientIP(), primaryLogin.getId().getLogin(),
					primaryLogin.getId().getDomainId());
		...
		if (resp.getStatus() == ResponseStatus.SUCCESS) {
			auditLog = auditHelper.addLog("CREATE", user.getRequestorDomain(),
					...
					primaryLogin.getId().getLogin(), primaryLogin.getId()
							.getDomainId());
		...
		try {
			decPassword = loginManager.decryptPassword(primaryLogin
					.getPassword());
		...
		if (user.isAddInitialPasswordToHistory() || customPassword) {
			...
			PasswordHistory hist = new PasswordHistory(primaryLogin.getId()
					.getLogin(), primaryLogin.getId().getDomainId(),
		...
		passwordPolicy = passwordDS.getPasswordPolicy(primaryLogin.getId()
				.getDomainId(), primaryLogin.getId().getLogin(), primaryLogin
				.getId().getManagedSysId());
				...
				for (Resource res : resourceList) {
					...
					if (managedSysId != null && managedSysId.length() > 0) {
						...
						if (!mngSysIdentityExists) {
							...
							String newPrincipalName = buildPrincipalName(
									attrMap, se, bindingMap);
							log.debug(" - New principalName = "
									+ newPrincipalName);
							resLogin = new Login();
							// get the current object as it stands in the target
							// system
							LoginId resLoginId = new LoginId(primaryLogin
									.getId().getDomainId(), newPrincipalName,
									managedSysId);
						...
						if (!userExistedInTargetSystem) {
							...
							bindingMap.put(TARGET_SYSTEM_IDENTITY, resLogin
									.getId().getLogin());
							...
							ExtensibleUser extUser = buildModifyFromRules(user,
									resLogin, attrMap, se, managedSysId,
									resLogin.getId().getDomainId(), bindingMap,
									user.getCreatedBy());
							...
							if (connector.getConnectorInterface() != null
								...
								userReq.setUserIdentity(resLogin.getId()
										.getLogin());
		...
		// make sure that we have an identity for each domain that this user
		// belongs to
		validateIdentitiesExistforSecurityDomain(primaryLogin,
				user.getMemberOfRoles());
		...
		if (!connectorSuccess) {
			...
			if (user.isEmailCredentialsToNewUsers()) {
				sendCredentialsToUser(user, primaryLogin.getId().getLogin(),
						decPassword);
			}
				...
				if (user.getSupervisor() != null) {
					...
					if (sv != null && sv.getSupervisor() != null) {
						sendCredentialsToSupervisor(sv.getSupervisor(),
								primaryLogin.getId().getLogin(), decPassword,
	...
	public ProvisionUserResponse deleteByUserId(ProvisionUser user,
		...
		if (lRequestor != null && lTargetUser != null) {
			...
			auditLog = auditHelper.addLog("DELETE", lRequestor.getId()
					.getDomainId(), lRequestor.getId().getLogin(),
					"IDM SERVICE", usr.getCreatedBy(), "0", "USER", usr
							.getUserId(), null, "SUCCESS", null, "USER_STATUS",
					usr.getStatus().toString(), requestId, null, null, null,
				...
				if (l.getStatus() == null
					...
					if (user.isNotifyTargetSystems()) {
						...
						if (!l.getId()
							...
							ManagedSys mSys = managedSysService.getManagedSys(l
									.getId().getManagedSysId());
							ProvisionConnector connector = connectorService
									.getConnector(mSys.getConnectorId());
							...
							PSOIdentifierType idType = new PSOIdentifierType(l
							...
							if (connector.getConnectorInterface() != null
								...
								remoteDelete(l, requestId, mSys, connector,
										matchObj, user, auditLog);

							} else {

								localDelete(l, requestId, idType, mSys, user,
										auditLog);
		...
		if (!managedSystemId.equalsIgnoreCase(sysConfiguration
			...
			if (connector.getConnectorInterface() != null
					&& connector.getConnectorInterface().equalsIgnoreCase(
							"REMOTE")) {
				UserResponse resp = remoteDelete(login, requestId, mSys,
						connector, matchObj, new ProvisionUser(usr), auditLog);
				if (resp.getStatus() == StatusCodeType.SUCCESS) {
					connectorSuccess = true;
				}
			} else {
				ResponseType resp = localDelete(login, requestId, idType, mSys,
			...
			if (lRequestor != null && lTargetUser != null) {
				...
				auditLog = auditHelper.addLog("DELETE", lRequestor.getId()
						...
						"USER_STATUS", usr.getStatus().toString(), requestId,
						null, null, null, null, lTargetUser.getId().getLogin(),
						lTargetUser.getId().getDomainId());
			} else {
					...
					if (l.getStatus() != null
						...
						if (!l.getId()
							...
							if (connector.getConnectorInterface() != null
									&& connector.getConnectorInterface()
											.equalsIgnoreCase("REMOTE")) {
								UserResponse resp = remoteDelete(login,
								...
								ResponseType resp = localDelete(login,
										requestId, idType, mSys, pUser,
										auditLog);
	...
	public Response lockUser(String userId, AccountLockEnum operation,
		...
		final String logDomainId = (lRequestor != null && lRequestor.getId() != null) ? lRequestor
				.getId().getDomainId() : null;
		final String logLoginId = (lRequestor != null && lRequestor.getId() != null) ? lRequestor
				.getId().getLogin() : null;
		...
		for (final Login userLogin : loginList) {
			if (userLogin != null) {
				...
				if (id != null && id.getManagedSysId() != null
						&& !id.getManagedSysId().equals("0")) {
					ResponseType responsetype = null;
					final String managedSysId = id.getManagedSysId();
					final ManagedSys managedSys = managedSysService
							.getManagedSys(managedSysId);
					final PSOIdentifierType psoIdentifierType = new PSOIdentifierType(
							id.getLogin(), null, managedSysId);
		...
		final List<Role> roleList = roleDataService.getUserRoles(user
				.getUserId());
		if (CollectionUtils.isNotEmpty(roleList)) {
			for (final Role role : roleList) {
				final RoleId roleId = role.getId();
				final List<Resource> resourceList = resourceDataService
						.getResourcesForRole(roleId.getServiceId(),
								roleId.getRoleId());
	...
	public ProvisionUserResponse modifyUser(ProvisionUser pUser) {
		...
		List<Role> curRoleList = roleDataService.getUserRolesAsFlatList(pUser
				.getUserId());
		List<Group> curGroupList = groupManager.getUserInGroupsAsFlatList(pUser
				.getUserId());
		...
		updatePrincipalList(origUser.getUserId(), curPrincipalList,
				newPrincipalList, deleteResourceList, principalList);
		...
		IdmAuditLog auditLog = auditHelper.addLog("MODIFY",
				...
				pUser.getRequestClientIP(), primaryIdentity.getId().getLogin(),
				primaryIdentity.getId().getDomainId());
			...
			for (Resource res : resourceList) {
				...
				if (managedSysId != null) {
					...
					if (mLg != null && mLg.getId() != null) {
						bindingMap.put(TARGET_SYS_SECURITY_DOMAIN, mLg.getId()
								.getDomainId());
					} else {
						bindingMap.put(TARGET_SYS_SECURITY_DOMAIN,
								sysConfiguration.getDefaultSecurityDomain());
					...
					if (mLg != null
							&& mLg.getOperation() == AttributeOperationEnum.DELETE) {
						...
						if (mLg == null
								|| (mLg != null && !isExistedInTargetSystem)) {
							...
							if (mLg == null) {
								...
								log.debug(" - New principalName = "
										+ newPrincipalName);
								mLg = new Login();
								// get the current object as it stands in the
								// target
								// system
								LoginId resLoginId = new LoginId(
										primaryIdentity.getId().getDomainId(),
										newPrincipalName, managedSysId);
							...
							bindingMap.put(TARGET_SYS_SECURITY_DOMAIN, mLg
									.getId().getDomainId());
							...
							if (connectorSuccess) {
								 ...
								 * .getDomainId(), mLg.getId() .getLogin(),
								 * mLg.getId() .getManagedSysId());
								 */
		...
		validateIdentitiesExistforSecurityDomain(primaryIdentity,
				activeRoleList);
				...
				if (mLg != null) {
					...
					try {
						mLg.setPassword(loginManager
								.encryptPassword(scrambledPassword));
	...
	public PasswordResponse resetPassword(PasswordSync passwordSync) {
		...
		if (passwordSync.getManagedSystemId().equalsIgnoreCase(
			...
			// typical sync
			List<Login> principalList = loginManager.getLoginByUser(login
					.getUserId());
			if (principalList != null) {
			...
			ManagedSys mSys = managedSysService.getManagedSys(passwordSync
					.getManagedSystemId());
			ProvisionConnector connector = connectorService.getConnector(mSys
					.getConnectorId());
	...
	// public LookupUserResponse getAllUsers(String managedSysId) {
	//
	// LookupUserResponse response = new LookupUserResponse(
	// ResponseStatus.SUCCESS);
	// response.setManagedSysId(managedSysId);
	...
	// reqType.setPsoID(idType);
	//
	// reqType.setSearchValue("");
	// reqType.setSearchQuery(searchString);
	// reqType.setTargetID(managedSysId);
	// reqType.setHostLoginId(mSys.getUserId());
	// reqType.setHostLoginPassword(mSys.getDecryptPassword());
	// reqType.setHostUrl(mSys.getHostUrl());
	...
	public Response setPassword(PasswordSync passwordSync) {
		...
		String encPassword = null;
		try {
			encPassword = loginManager.encryptPassword(passwordSync
					.getPassword());
		} catch (EncryptionException e) {
			response.setStatus(ResponseStatus.FAILURE);
			response.setErrorCode(ResponseCode.FAIL_ENCRYPTION);
		...
		for (Login l : principalList) {
			...
			if (l.getId().getManagedSysId()
				...
				boolean retval = loginManager.setPassword(l.getId()
						.getDomainId(), l.getId().getLogin(), passwordSync
						.getManagedSystemId(), encPassword);
				if (retval) {
					log.debug("-Password changed in openiam repository for user:"
							+ passwordSync.getPrincipal());
		...
		if (passwordSync.getManagedSystemId().equalsIgnoreCase(
			...
			for (Login lg : principalList) {
				...
				if (!lg.getId()
					...
					if (syncAllowed(res)) {
						...
						loginManager.setPassword(lg.getId().getDomainId(), lg
								.getId().getLogin(), lg.getId()
								.getManagedSysId(), encPassword);
						...
						if (mSys.getConnectorId() != null
							...
							if (connector.getConnectorInterface() != null
								...
								org.openiam.connector.type.ResponseType resp = remoteSetPassword(
			...
			// pre-process
			Resource resource = resourceDataService.getResource(mSys
					.getResourceId());
	...
	public Response syncPasswordFromSrc(PasswordSync passwordSync) {
		...
		for (Login l : principalList) {
			...
			if (l.getId().getManagedSysId()
				...
				boolean retval = loginManager.setPassword(l.getId()
						.getDomainId(), l.getId().getLogin(), l.getId()
						.getManagedSysId(), encPassword);
				if (retval) {
					log.debug("-Password changed in openiam repository for user:"
							+ passwordSync.getPrincipal());
				...
				if (syncAllowed(res)) {
					...
					// update the password in openiam
					loginManager.setPassword(l.getId().getDomainId(), l.getId()
							.getLogin(), l.getId().getManagedSysId(),
							encPassword);
	...
	private boolean syncAllowed(Resource res) {
		...
		if (res == null) {
			...
			log.debug("Checking if password sync allowed for resource="
					+ res.getResourceId());
	...
	private List<Resource> getResourcesForRole(List<Role> roleList) {
		...
		for (Role rl : roleList) {
			...
			if (rl != null) {
				...
				log.debug("-Adding role id to list of roles:"
						+ rl.getId().getRoleId());
		...
		if (domainId != null && roleIdList != null) {
			...
			return resourceDataService.getResourcesForRoles(domainId,
					roleIdList);
		}
		return null;
	}
