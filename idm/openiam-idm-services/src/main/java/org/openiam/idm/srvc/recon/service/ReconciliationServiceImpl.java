...
public class ReconciliationServiceImpl implements ReconciliationService,
	...
	public ReconciliationResponse startReconciliation(
		...
		try {
			...
			if (connector.getServiceUrl().contains("CSV")) {
				log.debug("Start CSV recon");
				connectorAdapter.reconcileResource(mSys, config, muleContext);
				log.debug("end CSV recon");
				return new ReconciliationResponse(ResponseStatus.SUCCESS);
			}

			// Check if exist any user records with Login for Msys in IDM
			// if exist run first recon part
			...
			if (!CollectionUtils.isEmpty(principalList)) {
				...
				// Do reconciliation User from Target Managed System to IDM,
				// search for all Roles and Groups related with resource
				for (ResourceRole rRole : res.getResourceRoles()) {
					List<String> usrIds = roleDataService.getUsersInRoleIds(
			...
			// Second recon part. We run it anyway
	...
	private boolean reconciliationUser(final String usrId,
			final ManagedSys mSys,
			...
			for (Login login : logins) {
				if (login.getId().getDomainId()
						...
						&& login.getId().getManagedSysId()
								.equalsIgnoreCase(mSys.getManagedSysId())) {
					l = login;
					break;
				}
			}
		...
		if (l == null) {
			if (user.getStatus().equals(UserStatusEnum.DELETED)) {
				// User is deleted and has no Identity for this managed system
				// -> goto next user
				return true;
			}
		...
		// User user = userMgr.getUserByPrincipal(l.getId().getDomainId(),
