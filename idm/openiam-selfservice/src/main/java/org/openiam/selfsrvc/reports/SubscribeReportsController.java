    ...
    protected ModelAndView onSubmit(HttpServletRequest request,
        ...
        if (request.getParameterMap().containsKey("save")) {
            ...
            if(!StringUtils.isEmpty(reportCommand.getReport().getReportName())) {
                reportService.createOrUpdateSubscribedReportInfo(reportCommand.getReport().getReportName(), dataSourceFile.getSize() > 0 ? dataSourceFileName : reportCommand.getReport().getReportDataSource(), designFile.getSize() > 0 ? designFileName : reportCommand.getReport().getReportUrl(), null);
            }
        }
