...
public class UserMgr implements UserDataService {
	...
	public User addUserWithDependent(User user, boolean dependency) {
		...
		/*
		 * if (!dependency) return user;
		 */

		// address
		/*
		 * Map<String, Address> adrMap = user.getAddresses(); if (adrMap != null
		 * && adrMap.size() > 0 ) {
	...
	public List<User> searchByDelegationProperties(DelegationFilterSearch search) {
		List<UserEntity> entityList = userDao
				.findByDelegationProperties(search);
	...
	public UserAttribute addAttribute(UserAttribute attribute) {
		...
		UserAttributeEntity attributeEntity = userAttributeDozerConverter
	...
	public void updateAttribute(UserAttribute attribute) {
		...
		userAttributeDao.update(userAttributeDozerConverter.convertToEntity(
				attribute, true));
	...
	public UserAttribute getAttribute(String attrId) {
		...
		return attributeEntity != null ? new UserAttribute(attributeEntity)
				: null;
	}
	...
	public void removeAttribute(UserAttribute attr) {
		...
		userAttributeDao.remove(userAttributeDozerConverter.convertToEntity(
				attr, true));
	...
	public Map<String, UserAttribute> getUserAsMap(String userId) {
		...
		attrMap.put("USER_ID", new UserAttribute(null, userId, null, "USER_ID",
				userId));
		attrMap.put("FIRST_NAME", new UserAttribute(null, userId, null,
				"FIRST_NAME", usr.getFirstName()));
		attrMap.put("LAST_NAME", new UserAttribute(null, userId, null,
				"LAST_NAME", usr.getLastName()));
		attrMap.put("MIDDLE_INIT", new UserAttribute(null, userId, null,
				"MIDDLE_INIT", String.valueOf(usr.getMiddleInit())));
		attrMap.put("TITLE",
				new UserAttribute(null, userId, null, "TITLE", usr.getTitle()));
		attrMap.put("DEPT",
				new UserAttribute(null, userId, null, "DEPT", usr.getDeptCd()));
		attrMap.put("STATUS", new UserAttribute(null, userId, null, "STATUS",
				usr.getStatus().toString()));
		if (usr.getBirthdate() != null) {
			attrMap.put("BIRTHDATE", new UserAttribute(null, userId, null,
					"BIRTHDATE", usr.getBirthdate().toString()));
		} else {
			attrMap.put("BIRTHDATE", new UserAttribute(null, userId, null,
					"BIRTHDATE", null));
		}
		attrMap.put(
				"SEX",
				new UserAttribute(null, userId, null, "SEX", String.valueOf(usr
						.getSex())));
		if (usr.getCreateDate() != null) {
			attrMap.put("CREATE_DATE", new UserAttribute(null, userId, null,
					"CREATE_DATE", usr.getCreateDate().toString()));
		} else {
			attrMap.put("CREATE_DATE", new UserAttribute(null, userId, null,
					"CREATE_DATE", null));

		}
		attrMap.put("CREATED_BY", new UserAttribute(null, userId, null,
				"CREATED_BY", usr.getCreatedBy()));
		if (usr.getLastUpdate() != null) {
			attrMap.put("LAST_UPDATE", new UserAttribute(null, userId, null,
					"LAST_UPDATE", usr.getLastUpdate().toString()));
		} else {
			attrMap.put("LAST_UPDATE", new UserAttribute(null, userId, null,
					"LAST_UPDATE", null));

		}
		attrMap.put("LAST_UPDATEDBY", new UserAttribute(null, userId, null,
				"LAST_UPDATEDBY", usr.getLastUpdatedBy()));
		attrMap.put("PREFIX", new UserAttribute(null, userId, null, "PREFIX",
				usr.getPrefix()));
		attrMap.put("SUFFIX", new UserAttribute(null, userId, null, "SUFFIX",
				usr.getSuffix()));
		attrMap.put("USER_TYPE_IND", new UserAttribute(null, userId, null,
				"USER_TYPE_IND", usr.getUserTypeInd()));
		attrMap.put("EMPLOYEE_ID", new UserAttribute(null, userId, null,
				"EMPLOYEE_ID", usr.getEmployeeId()));
		attrMap.put("EMPLOYEE_TYPE", new UserAttribute(null, userId, null,
				"EMPLOYEE_TYPE", usr.getEmployeeType()));
		attrMap.put("LOCATION_ID", new UserAttribute(null, userId, null,
				"LOCATION_ID", usr.getLocationCd()));
		attrMap.put("ORGANIZATION_ID", new UserAttribute(null, userId, null,
				"ORGANIZATION_ID", usr.getCompanyId()));
		attrMap.put("COMPANY_OWNER_ID", new UserAttribute(null, userId, null,
				"COMPANY_OWNER_ID", usr.getCompanyOwnerId()));

		attrMap.put("MANAGER_ID", new UserAttribute(null, userId, null,
				"MANAGER_ID", usr.getManagerId()));
	...
	public List<UserAttribute> getUserAsAttributeList(String principalName,
		...
		User u = getUserByPrincipal(
				sysConfiguration.getDefaultSecurityDomain(), principalName,
				sysConfiguration.getDefaultManagedSysId(), false);
		if (u == null) {
			return null;
		}
	...
	public Address addAddress(Address val) {
		...
		AddressEntity addressEntity = addressDao.add(addressDozerConverter
	...
	public Address getAddressByName(String userId, String addressName) {
		...
		AddressEntity addressEntity = addressDao.findByName(addressName,
				userId, ContactConstants.PARENT_TYPE_USER);
		return addressEntity != null ? addressDozerConverter.convertToDTO(
				addressEntity, true) : null;
	}
	...
	public Address getDefaultAddress(String userId) {
		if (userId == null)
			throw new NullPointerException("userId is null");
		AddressEntity addressEntity = addressDao.findDefault(userId,
	...
	public List<Address> getAddressList(String userId) {
		...
		List<AddressEntity> addressEntityList = addressDao.findByParentAsList(
				userId, ContactConstants.PARENT_TYPE_USER);
	...
	public Map<String, Address> getAddressMap(String userId) {
		...
		Map<String, AddressEntity> addressEntityMap = addressDao.findByParent(
				userId, ContactConstants.PARENT_TYPE_USER);
		if (addressEntityMap != null) {
			addressMap = new HashMap<String, Address>();
			for (Map.Entry<String, AddressEntity> addressEntityEntry : addressEntityMap
					.entrySet()) {
				addressMap.put(
						addressEntityEntry.getKey(),
						addressDozerConverter.convertToDTO(
								addressEntityEntry.getValue(), true));
			}
		}
		return addressMap;
	}
	...
	public Phone addPhone(Phone val) {
		...
		val.setParentType(ContactConstants.PARENT_TYPE_USER);
		PhoneEntity phoneEntity = phoneDao.add(phoneDozerConverter
				.convertToEntity(val, true));
	 ...
	 * 
	 * @see
	 * org.openiam.idm.srvc.user.service.UserDataService#getPhoneByName(java
	 * .lang.String, java.lang.String)
	 */
	@Transactional(readOnly = true)
	 ...
	 * 
	 * @see
	 * org.openiam.idm.srvc.user.service.UserDataService#getDefaultPhone(java
	 * .lang.String)
	 */
	@Transactional(readOnly = true)
	public Phone getDefaultPhone(String userId) {
		...
		PhoneEntity phoneEntity = phoneDao.findDefault(userId,
				ContactConstants.PARENT_TYPE_USER);
		return phoneDozerConverter.convertToDTO(phoneEntity, true);
	}
	...
	public Map<String, Phone> getPhoneMap(String userId) {
		...
		Map<String, PhoneEntity> phoneEntityMap = phoneDao.findByParent(userId,
				ContactConstants.PARENT_TYPE_USER);
		if (phoneEntityMap != null) {
			phoneMap = new HashMap<String, Phone>();
			for (Map.Entry<String, PhoneEntity> phoneEntityEntry : phoneEntityMap
					.entrySet()) {
				phoneMap.put(
						phoneEntityEntry.getKey(),
						phoneDozerConverter.convertToDTO(
								phoneEntityEntry.getValue(), true));
			}
		}
		return phoneMap;
	}
	...
	public EmailAddress addEmailAddress(EmailAddress val) {
		...
		val.setParentType(ContactConstants.PARENT_TYPE_USER);
		EmailAddressEntity emailAddressEntity = emailAddressDozerConverter
				.convertToEntity(val, true);
		emailAddressEntity = emailAddressDao.add(emailAddressEntity);
		return emailAddressDozerConverter
				.convertToDTO(emailAddressEntity, true);
	...
	public void updateEmailAddress(EmailAddress val) {
		...
		emailAddressDao.update(emailAddressDozerConverter.convertToEntity(val,
				true));
	}
	...
	public void removeEmailAddress(EmailAddress val) {
		...
		emailAddressDao.remove(emailAddressDozerConverter.convertToEntity(val,
				true));
	}
	...
	public EmailAddress getEmailAddressById(String addressId) {
		if (addressId == null)
			throw new NullPointerException("addressId is null");
		EmailAddressEntity emailAddressEntity = emailAddressDao
				.findById(addressId);
		return emailAddressDozerConverter
				.convertToDTO(emailAddressEntity, true);
	}
	...
	public EmailAddress getEmailAddressByName(String userId, String addressName) {
		...
		EmailAddressEntity emailAddressEntity = emailAddressDao.findByName(
				addressName, userId, ContactConstants.PARENT_TYPE_USER);
		return emailAddressDozerConverter
				.convertToDTO(emailAddressEntity, true);
	}
	...
	public EmailAddress getDefaultEmailAddress(String userId) {
		...
		EmailAddressEntity emailAddressEntity = emailAddressDao.findDefault(
				userId, ContactConstants.PARENT_TYPE_USER);
		return emailAddressDozerConverter
				.convertToDTO(emailAddressEntity, true);
	}
	...
	public List<EmailAddress> getEmailAddressList(String userId) {
		...
		List<EmailAddressEntity> emailAddressEntityList = emailAddressDao
				.findByParentAsList(userId, ContactConstants.PARENT_TYPE_USER);

		return emailAddressDozerConverter.convertToDTOList(
				emailAddressEntityList, false);
	}
	...
	public Map<String, EmailAddress> getEmailAddressMap(String userId) {
		...
		Map<String, EmailAddressEntity> emailAddressEntityMap = emailAddressDao
				.findByParent(userId, ContactConstants.PARENT_TYPE_USER);
		Map<String, EmailAddress> emailAddressMap = new HashMap<String, EmailAddress>();
		for (Map.Entry<String, EmailAddressEntity> addressEntityEntry : emailAddressEntityMap
				.entrySet()) {
			emailAddressMap.put(
					addressEntityEntry.getKey(),
					emailAddressDozerConverter.convertToDTO(
							addressEntityEntry.getValue(), true));
		}
	...
	public Supervisor addSupervisor(Supervisor supervisor) {
		...
		SupervisorEntity supervisorEntity = supervisorDozerConverter
	...
	public void updateSupervisor(Supervisor supervisor) {
		if (supervisor == null)
			throw new NullPointerException("supervisor is null");
		this.supervisorDao.update(supervisorDozerConverter.convertToEntity(
				supervisor, true));
	}
	...
	public Supervisor getSupervisor(String supervisorObjId) {
		...
		return supervisorDozerConverter.convertToDTO(
				supervisorDao.findById(supervisorObjId), true);
	}
	...
	public List<Supervisor> getSupervisors(String employeeId) {
		...
		List<SupervisorEntity> superVisList = supervisorDao
				.findSupervisors(employeeId);
		...
		for (SupervisorEntity sup : superVisList) {
			...
			org.hibernate.Hibernate.initialize(sup.getSupervisor()
					.getEmailAddresses());
			org.hibernate.Hibernate.initialize(sup.getSupervisor()
					.getAddresses());
			org.hibernate.Hibernate.initialize(sup.getSupervisor()
					.getUserAttributes());
			supervisorList
					.add(supervisorDozerConverter.convertToDTO(sup, true));
		}
	 ...
	 * 
	 * @see
	 * org.openiam.idm.srvc.user.service.UserDataService#getEmployees(java.lang
	 * .String)
	 */
	@Transactional(readOnly = true)
	public List<Supervisor> getEmployees(String supervisorId) {
		if (supervisorId == null)
			throw new NullPointerException("employeeId is null");
		List<SupervisorEntity> superVisList = supervisorDao
				.findEmployees(supervisorId);
		List<Supervisor> supervisorList = new LinkedList<Supervisor>();
		// initialize the collections dependant objects
		for (SupervisorEntity sup : superVisList) {
			org.hibernate.Hibernate.initialize(sup.getEmployee().getPhones());
			org.hibernate.Hibernate.initialize(sup.getEmployee()
					.getEmailAddresses());
			org.hibernate.Hibernate
					.initialize(sup.getEmployee().getAddresses());
			org.hibernate.Hibernate.initialize(sup.getEmployee()
					.getUserAttributes());
			supervisorList
					.add(supervisorDozerConverter.convertToDTO(sup, true));
	...
	public Supervisor getPrimarySupervisor(String employeeId) {
		if (employeeId == null)
			throw new NullPointerException("employeeId is null");
		SupervisorEntity entity = supervisorDao
				.findPrimarySupervisor(employeeId);
	 ...
	 * 
	 * @see
	 * org.openiam.idm.srvc.user.service.UserDataService#setUserDao(org.openiam
	 * .idm.srvc.user.service.UserDAO)
	 */
	 ...
	 * 
	 * @see
	 * org.openiam.idm.srvc.user.service.UserDataService#getUserAttributeDao()
	 */
	 ...
	 * 
	 * @see
	 * org.openiam.idm.srvc.user.service.UserDataService#setUserAttributeDao
	 * (org.openiam.idm.srvc.user.service.UserAttributeDAO)
	 */
	 ...
	 * 
	 * @see
	 * org.openiam.idm.srvc.user.service.UserDataService#setUserNoteDao(org.
	 * openiam.idm.srvc.user.service.UserNoteDAO)
	 */
	 ...
	 * 
	 * @see
	 * org.openiam.idm.srvc.user.service.UserDataService#setAddressDao(org.openiam
	 * .idm.srvc.continfo.service.AddressDAO)
	 */
	 ...
	 * 
	 * @see
	 * org.openiam.idm.srvc.user.service.UserDataService#getEmailAddressDao()
	 */
	 ...
	 * 
	 * @see
	 * org.openiam.idm.srvc.user.service.UserDataService#setEmailAddressDao(
	 * org.openiam.idm.srvc.continfo.service.EmailAddressDAO)
	 */
	 ...
	 * 
	 * @see
	 * org.openiam.idm.srvc.user.service.UserDataService#setPhoneDao(org.openiam
	 * .idm.srvc.continfo.service.PhoneDAO)
	 */
	 ...
	 * 
	 * @see
	 * org.openiam.idm.srvc.user.service.UserDataService#setSupervisorDao(org
	 * .openiam.idm.srvc.user.service.SupervisorDAO)
	 */
