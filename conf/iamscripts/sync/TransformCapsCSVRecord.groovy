...
public class TransformCapsCSVRecord extends AbstractTransformScript {
    ...
    def addEmailAddress(ProvisionUser pUser, EmailAddress emailAddress)     {
        ...
        if (!isNewUser) {
            ...
            emailAddresses = userManager.getEmailAddressDtoList(user.id, false)
        }
        if (emailAddresses) {
    ...
    def addOCOrganization(ProvisionUser pUser, String homeDeptCd) {
        ...
        orgSearchBean.name = "County of Orange"
        orgSearchBean.organizationTypeId = "ORGANIZATION"
        def orgList = organizationService.findBeans(orgSearchBean, null, 0, 1, null)
        if (orgList) {
        ...
        if (homeDeptCd) {
            ...
            def deptList = organizationService.findBeans(deptSearchBean, null, 0, 1, null)
            if (deptList) {
    ...
    def addRole(ProvisionUser pUser, String roleName) {
        def foundRole = userRoleList.find { r-> r.name == roleName }
        if (!foundRole) {
    ...
    def addAddress(ProvisionUser pUser, Address address) {
        ...
        if (!isNewUser) {
            def userManager = context?.getBean("userManager") as UserDataService
            addresses = userManager.getAddressDtoList(user.id, false)
        }
        if (addresses) {
    ...
    def addPhone(ProvisionUser pUser, Phone phone) {
        ...
        if (!isNewUser) {
            def userManager = context?.getBean("userManager") as UserDataService
            phones = userManager.getPhoneDtoList(user.id, false)
        }
        if (phones) {
            pUser.phones = phones
        }
