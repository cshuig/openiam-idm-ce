...
supervisorSet?.each { s->
    ...
    def identity = loginManager.getByUserIdManagedSys(s.getId(), managedSysId)
    if (identity) {
        println("Adding supervisor id " + s.getId() + " --> " + identity.login)
        attributeContainer.attributeList.add(new BaseAttribute(identity.login, identity.login, s.operation))
    } else {
        println("Adding supervisor id " + s.getId() + " failed, no identity found for supervisor user")
    }
}
oldSupervisorSet?.each { s->
    if (!(s in supervisorSet)) {
        ...
        def identity = loginManager.getByUserIdManagedSys(s.getId(), managedSysId)
        if (identity) {
            println("Deleting supervisor id " + s.getId() + " --> " + identity.login)
            attributeContainer.attributeList.add(new BaseAttribute(identity.login, identity.login, s.operation))
        } else {
            println("Deleting supervisor id " + s.getId() + " failed, no identity found for supervisor user")
        }
    }
}
