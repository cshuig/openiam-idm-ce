...
if (binding.hasVariable("managedSysId")) {
    ...
    def lookupUserResponse = groupProvisionService.getTargetSystemObject(groupIdentity.identity, managedSysId, [new ExtensibleAttribute('member', null)])
    ...
    for(UserEntity member : members) {
        ...
        if (principal) {
            def resp = provisionService.getTargetSystemUser(principal, managedSysId, [])
            if (resp.success && resp.attrList) {
                def identityDN = resp.attrList.get(0)?.value // DN attribute should be in place by default
                if (identityDN) {
