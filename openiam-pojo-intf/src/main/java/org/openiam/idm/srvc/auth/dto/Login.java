...
@XmlType(name = "Login", propOrder = { "id", "userId", "password",
		"pwdEquivalentToken", "pwdChanged", "pwdExp", "firstTimeLogin",
		"resetPassword", "isLocked", "status", "gracePeriod", "createDate",
		"createdBy", "currentLoginHost", "authFailCount", "lastAuthAttempt",
		"canonicalName", "lastLogin", "isDefault", "selected",
		"loginAttributes", "passwordChangeCount", "operation",
		"origPrincipalName", "managedSysName", "lastLoginIP", "prevLoginIP",
		"prevLogin", "pswdResetToken", "pswdResetTokenExp",
		"loginAttrInTargetSystem" })
@XmlSeeAlso({ Subject.class, SSOToken.class })
...
public class Login implements java.io.Serializable, Cloneable {
	...
	protected Set<LoginAttribute> loginAttributes = new HashSet<LoginAttribute>(
			0);
	...
	public Login(LoginId id, String userId, String password,
			String pwdEquivalentToken, Date pwdChanged, Date pwdExp,
			int resetPwd, int isLocked, String status, Date gracePeriod,
			Date createDate, String createdBy, String currentLoginHost,
			Integer authFailCount, Date lastAuthAttempt,
	...
	public Object clone() {
		Login l = new Login();
		LoginId lgId = new LoginId(id.getDomainId(), id.getLogin(),
				id.getManagedSysId());
		l.setId(lgId);
	...
	public String toString() {
		return "Login{" + "operation=" + operation + ", id=" + id
				...
				+ ", selected=" + selected + ", loginAttributes="
				+ loginAttributes + ", origPrincipalName='" + origPrincipalName
				+ '\'' + ", managedSysName='" + managedSysName + '\''
				+ ", lastLoginIP='" + lastLoginIP + '\'' + ", prevLogin="
				+ prevLogin + ", prevLoginIP='" + prevLoginIP + '\'' + '}';
	}
	...
	public boolean equals(Object o) {
		...
		if (selected != login.selected)
			return false;
		if (canonicalName != null ? !canonicalName.equals(login.canonicalName)
				: login.canonicalName != null)
			return false;
		if (id != null ? !id.equals(login.id) : login.id != null)
			return false;
		if (managedSysName != null ? !managedSysName
				.equals(login.managedSysName) : login.managedSysName != null)
			return false;
		if (origPrincipalName != null ? !origPrincipalName
				.equals(login.origPrincipalName)
				: login.origPrincipalName != null)
			return false;
		if (status != null ? !status.equals(login.status)
				: login.status != null)
			return false;
		if (userId != null ? !userId.equals(login.userId)
				: login.userId != null)
			return false;
	...
	public int hashCode() {
		...
		result = 31 * result
				+ (canonicalName != null ? canonicalName.hashCode() : 0);
		result = 31 * result + (selected ? 1 : 0);
		result = 31
				* result
				+ (origPrincipalName != null ? origPrincipalName.hashCode() : 0);
		result = 31 * result
				+ (managedSysName != null ? managedSysName.hashCode() : 0);
		return result;
	}
}
